...
```bash
arkanum start 6 "what files are in the root?"
```
...
```bash
uv run python -m quests.quest_06_tool_chest.starter.main "what files are in the root?"
```
...
- los 4 schemas (`get_files_info`, `get_file_content`, `write_file`, `run_python_file`) están declarados
- las tools están registradas en `available_functions`
- el agente solicita herramientas (`function_calls`) cuando corresponde
- el system prompt describe las operaciones disponibles
