...
| H-08 | todos | Wrap de `rich.Console` rompe la búsqueda `expected in output` en `check.py` cuando la terminal es estrecha | ✅ Fix validado en Q05 (aplicar a 7 más) |
| H-09 | todos | UX confusa: la tabla pre-check toda en verde + panel rojo con solo el primer faltante. Aprendiz no sabe qué validación específica falló | ❌ pendiente |
...
| H-14 | Q06 | Output esquemático en consola: solo `Calling function:`, sin pensamiento ni explicación | ✅ Fix parcial validado en rama (pensamiento via `response.text`; el "resultado de la tool" requiere Q07) |
...
| H-22 | Q07 | Ni el docstring del starter ni el dashboard `/live-agent` aclaran que Q07 **no** devuelve el resultado al modelo. El README sí, pero quien se salta el README ve un trace que parece "incompleto" | ❌ pendiente |
...
| H-16 | docs | Inconsistencias menores en READMEs vs código real (mensajes de error, idioma, formato) | ❌ pendiente |
