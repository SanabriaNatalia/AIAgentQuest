...
| H-09 | todos | UX confusa: la tabla pre-check toda en verde + panel rojo con solo el primer faltante. Aprendiz no sabe qué validación específica falló | ✅ Fix validado en rama (`common/cli/check_runner.py` con `render_required_outputs_table` y `render_any_of_table`; los 8 check.py refactorizados) |
...
| H-22 | Q07 | Ni el docstring del starter ni el dashboard `/live-agent` aclaran que Q07 **no** devuelve el resultado al modelo. El README sí, pero quien se salta el README ve un trace que parece "incompleto" | ✅ Fix completo (docstring del starter con aviso ⚠ + banner informativo en `/live-agent` cuando el trace de Q07 termina sin `agent_final`) |
